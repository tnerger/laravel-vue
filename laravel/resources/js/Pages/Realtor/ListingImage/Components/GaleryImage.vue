<template>
    <div class="flex flex-col justify-end" :style="{ order: image.sort }">
...
const changeSort = (direction) => {
    router.put(route('realtor.listing.image.move', { listing: props.listing.id, image: props.image.id }),
        { direction: direction },
        {
            onSuccess: () => {
                emit('imageUpdated', props.image.id);
